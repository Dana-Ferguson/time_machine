...
class TimeAdjusters {
  ...
  static LocalTime Function(LocalTime) truncateToSecond = (time) => new LocalTime(time.hourOfDay, time.minuteOfHour, time.secondOfMinute);

  /// Gets a time adjuster to truncate the time to the minute, discarding fractional minutes.
  static LocalTime Function(LocalTime) truncateToMinute = (time) => new LocalTime(time.hourOfDay, time.minuteOfHour, 0);

  /// Get a time adjuster to truncate the time to the hour, discarding fractional hours.
  static LocalTime Function(LocalTime) truncateToHour = (time) => new LocalTime(time.hourOfDay, 0, 0);
}
